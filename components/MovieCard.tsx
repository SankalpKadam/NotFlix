...
const MovieCard: React.FC<MovieCardProps> = ({ data }) => {
    ...
    '>
            <img src={data.thumbnailUrl} alt="Thumbnail" className='cursor-pointer object-cover transition duration shadow-xl rounded-md group-hover:opacity-90 sm:group-hover:opacity-90 delay-300 w-full h-[12vw]' />
      ...
      group-hover:opacity-100'>
        ...
        '>
                    <div className='flex flex-row items-center gap-3'>
                        <div className='cursor-pointer w-6 h-6 lg:w-10 h-10 bg-white rounded-full flex justify-center items-center transition hover:bg-neutral-300' onClick={()=>{router.push("/watch")}}>
                            play
                            {/*insert play icon */}
                        </div>
                        ...
                        <div className='cursor-pointer ml-auto group/item w-6 h-6 lg:w-10 lg:h-10 border-white border-2 rounded-full flex justify-center items-center transition hover:border-neutral-300' onClick={()=> openModal(data.id)}>
                            V{/*chevron down icon text-white group-hover/item:text-neutral-300 size30*/}
                        </div>
                    </div>
